// Generated by zotero-plugin-scaffold
/* prettier-ignore */
/* eslint-disable */
// @ts-nocheck
export type FluentMessageId =
  | 'bold'
  | 'cancel'
  | 'confirm'
  | 'enable-richtext-hotkey'
  | 'enable-richtext-toolbar'
  | 'field-abbr'
  | 'help-version'
  | 'info-batch-break'
  | 'info-batch-finish'
  | 'info-batch-has-error'
  | 'info-batch-init'
  | 'info-batch-no-selected'
  | 'info-batch-running'
  | 'info-batch-stop-next'
  | 'info-noDOI'
  | 'italic'
  | 'lint-on-groupItem-added'
  | 'lint-on-item-added'
  | 'menuTools-label'
  | 'menuitem-label'
  | 'menuitem-stdFormatFlow'
  | 'no-case'
  | 'notify-on-lint'
  | 'prefs-title'
  | 'rule-correct-conference-abbr'
  | 'rule-correct-conference-abbr-description'
  | 'rule-correct-creators-case'
  | 'rule-correct-creators-case-menu-item'
  | 'rule-correct-creators-pinyin'
  | 'rule-correct-creators-pinyin-menu-item'
  | 'rule-correct-date-format'
  | 'rule-correct-date-format-menu-item'
  | 'rule-correct-pages-connector'
  | 'rule-correct-publication-title'
  | 'rule-correct-publication-title-menu-field'
  | 'rule-correct-publication-title-menu-item'
  | 'rule-correct-thesis-type'
  | 'rule-correct-university'
  | 'rule-no-article-webpage'
  | 'rule-no-article-webpage-report-message'
  | 'rule-no-doi-prefix'
  | 'rule-no-doi-prefix-menu-field'
  | 'rule-no-doi-prefix-menu-item'
  | 'rule-no-issue-extra-zeros'
  | 'rule-no-item-duplication'
  | 'rule-no-item-duplication-report-action'
  | 'rule-no-item-duplication-report-message'
  | 'rule-no-journal-preprint'
  | 'rule-no-journal-preprint-report-message'
  | 'rule-no-pages-extra-zeros'
  | 'rule-no-title-trailing-dot'
  | 'rule-no-volume-extra-zeros'
  | 'rule-require-journal-abbr'
  | 'rule-require-journal-abbr-menu-field'
  | 'rule-require-journal-abbr-menu-item'
  | 'rule-require-journal-abbr-option-choose-custom-abbr-data-button'
  | 'rule-require-journal-abbr-option-choose-custom-abbr-data-input'
  | 'rule-require-journal-abbr-option-custom-data-desc'
  | 'rule-require-journal-abbr-option-custom-data-label'
  | 'rule-require-journal-abbr-option-infer'
  | 'rule-require-journal-abbr-option-usefull'
  | 'rule-require-journal-abbr-option-usefull-en'
  | 'rule-require-journal-abbr-option-usefull-zh'
  | 'rule-require-language'
  | 'rule-require-language-menu-field'
  | 'rule-require-language-menu-item'
  | 'rule-require-language-option-only'
  | 'rule-require-language-option-only-cmn'
  | 'rule-require-language-option-only-eng'
  | 'rule-require-language-option-only-other'
  | 'rule-require-language-option-only-other-desc'
  | 'rule-require-language-option-only-other-doc'
  | 'rule-require-shortTitle-sentence-case'
  | 'rule-require-shortTitle-sentence-case-description'
  | 'rule-require-title-sentence-case'
  | 'rule-require-title-sentence-case-menu-field'
  | 'rule-require-title-sentence-case-menu-item'
  | 'rule-require-title-sentence-option-custom-term'
  | 'rule-require-title-sentence-option-custom-term-button'
  | 'rule-require-title-sentence-option-custom-term-desc'
  | 'rule-require-university-place'
  | 'rule-require-university-place-menu-field'
  | 'rule-require-university-place-menu-item'
  | 'rule-tool-creators-ext'
  | 'rule-tool-creators-ext-menu-item'
  | 'rule-tool-set-language'
  | 'rule-tool-set-language-menu-item'
  | 'rule-tool-title-guillemet'
  | 'rule-tool-title-guillemet-menu-item'
  | 'section-abbr'
  | 'section-creators'
  | 'section-date'
  | 'section-general'
  | 'section-identifier'
  | 'section-item'
  | 'section-language'
  | 'section-others'
  | 'section-pages'
  | 'section-publicationTitle'
  | 'section-thesis'
  | 'section-title'
  | 'section-updateMetadata'
  | 'small-caps'
  | 'subscript'
  | 'supscript'
  | 'tool-update-metadata'
  | 'tool-update-metadata-menu-item'
  | 'tool-update-metadata-option-confirm-when-itemtype-change'
  | 'tool-update-metadata-option-semanticScholarToken'
  | 'tool-update-metadata-option-semanticScholarToken-desc'
  | 'tool-update-metadata-option-semanticScholarToken-link'
  | 'unimplemented'
  | 'wip';
